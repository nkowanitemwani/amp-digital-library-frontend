  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API_BASE}/auth/grade/login`, {
        ...
        body: JSON.stringify({ school_id: form.schoolID.trim(), username: form.username.trim(), password: form.password }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (res.status === 429)      setError("Too many failed attempts. This account is temporarily locked. Please ask your teacher.");
        else if (res.status === 403) setError("This account has been deactivated. Please speak to your teacher.");
        else                         setError(data.error ?? "Incorrect username or password. Please try again.");
        return;
      }
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .step-num {
          ...
          font-size: 12px; font-weight: 700; color: #93C5FD; flex-shrink: 0; margin-top: 1px;
        }
        .format-pill {
        ...
        .dark-pill {
          ...
          border-radius: 100px; padding: 7px 16px; font-size: 12px; font-weight: 500; color: #CBD5E1;
      ...
      <div className="left-panel">
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "40px 0", position: "relative", zIndex: 1 }}>
          ...
          <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", fontWeight: 400, color: "#fff", lineHeight: 1.2, letterSpacing: "-0.025em", marginBottom: 12 }}>
            Your lessons,<br />
            <em style={{ color: "#FED7AA" }}>in three formats.</em>
          </h2>

          <p style={{ fontSize: 13, color: "#94A3B8", lineHeight: 1.7, marginBottom: 22, maxWidth: 310 }}>
            Sign in to access your school's library. For every unit your teacher has uploaded, you get three ways to learn.
          </p>
          ...
          {/* How to sign in */}
          <p style={{ fontSize: 10, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 24, marginBottom: 14 }}>
            How to sign in
          </p>
          ...
          <div className="step">
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "#64748B", marginTop: 3 }}>Your teacher has this — it's a string of letters and numbers</p>
            </div>
          </div>
          ...
          <div className="step">
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: "#E2E8F0" }}>Enter your grade username and password</p>
          ...
          <div className="step">
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: "#E2E8F0" }}>Listen, discuss, and take the quiz</p>
              <p style={{ fontSize: 12, color: "#64748B", marginTop: 3 }}>All three formats available for every unit</p>
            </div>
          </div>
        ...
        <div style={{ position: "relative", zIndex: 1, borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 20 }}>
          <p style={{ fontSize: 13, color: "#475569" }}>
            Are you a teacher?{" "}
            <Link href="/login" style={{ color: "#60A5FA", fontWeight: 600, textDecoration: "none" }}>Admin sign in →</Link>
          </p>
        </div>
      </div>
      ...
      <div className="right-panel" style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 32px", background: "#fff" }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <div style={{ marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 15, color: "#64748B" }}>Use your grade login details to access the library.</p>
          </div>
          ...
          <p style={{ textAlign: "center", fontSize: 14, color: "#64748B" }}>
            Teacher or admin?{" "}
            <Link href="/login" style={{ color: "#1D4ED8", fontWeight: 600, textDecoration: "none" }}>Admin sign in →</Link>
          </p>
        </div>
      </div>
    </div>
